...
func (s *LVMVolumeScheduler) filterForNewPVCs(pvcs []*corev1.PersistentVolumeClaim, node *corev1.Node) (bool, error) {
	...
	fmt.Println("debug filterForNewPVCs pvcs = %v", pvcs)
	...
	for i := range pvcs {
		...
		fmt.Println("debug constructLocalVolumeForPVC lv = %v, pvcs[i] = %v", lv, pvcs[i])
		if err != nil {
			return false, err
		}
	...
	fmt.Println("debug filterForNewPVCs nodeCandidates = %v, node.Name= %v, lvGroup[0] = %v", nodeCandidates, node.Name, lvGroup[0])
	if err != nil {
		return false, err
	}
